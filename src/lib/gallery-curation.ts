/* ----------------------------------------------------------------------------
   Flux Ads — gallery shaping (pure, isomorphic)

   Turns a raw Vercel Blob listing into the `Work[]` the UI renders. Runs both
   on the server (api/gallery.ts) and is the canonical home of the `Work` type.

   The gallery is now LIVE: whatever is in the Blob store shows up. This file
   only adds an optional *curation overlay* — hand-written title / category /
   aspect keyed by a file's stem (its name without extension). Files without an
   entry still appear; they just get an auto-derived title and flow at their
   natural size. To curate a new upload, add an entry to `curation` below.
---------------------------------------------------------------------------- */
...
export const stemOf = (pathname: string): string => {
  const base = pathname.slice(pathname.lastIndexOf('/') + 1)
  const dot = base.lastIndexOf('.')
  return dot === -1 ? base : base.slice(0, dot)
}

export const typeForFile = (pathname: string): MediaType => {
...
export const humanize = (stem: string): string => {
  const cleaned = stem
    .replace(/[_-]+/g, ' ')
...
/* Curation overlay. Keyed by file stem (name without extension). The `order`
   ...
   surfaced ahead of these, newest first. */
export const curation: Record<string, Meta> = {
...
export type BlobInput = { pathname: string; url: string; uploadedAt: number }

/**
 * Shape a raw Blob listing into ordered gallery works. New (uncurated) uploads
 * come first, newest first; curated items follow in their pinned order.
 */
export function shapeBlobs(blobs: BlobInput[]): Work[] {
  ...
  rows.sort((a, b) => {
    ...
    // Same bucket: uncurated → newest first; curated ties keep stable order.
